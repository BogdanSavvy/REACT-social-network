		...
		return (
			p <= 25 && (
				<span
					...
					className={p === props.currentPage ? `${style.pages__selected}` : null}
				>
					{p}
				</span>
			)
	...
	return (
		<div className={`${style.page}`}>
			<div className={`${style.page__container}`}>
				...
				<div className={`${style.page__users} ${style.users}`}>{members}</div>
				<div className={`${style.page__pages} ${style.pages}`}>{numbering}</div>
			</div>
		</div>
	);
};
