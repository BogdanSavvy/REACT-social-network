...
const Users = props => {
   ...
   let members = props.usersData.map(el => <Member key={el.id} id={el.id} bgImg={el.photos.large} ava={el.photos.small} name={el.name}
   ...
   let numbering = pageNumbers.map(p => p <= 25 && <span key={p} onClick={() => { props.onPageChaged(p) }} className={p === props.currentPage && `${style.pages__selected}`}>{p}</span>);
